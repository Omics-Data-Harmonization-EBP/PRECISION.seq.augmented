...
A precision object with RUVs harmonization results added to the slots
`harmon.train.data`, `harmon.test1.data`, and `harmon.test2.data` (if
applicable).

## Details

This function applies RUVs (centered (technical) replicate/negative
control samples) harmonization to the raw data contained in a precision
object. The harmonization results are added to the corresponding slots
in the object for each dataset ("train", "test1", "test2") if they
exist.
