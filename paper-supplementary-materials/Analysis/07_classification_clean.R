...
clean.scenario <- function(c){
  ...
  train.dataset <- lapply(clean.datasets, function(x) list(data = x$data[, c(1:100, 201:300)],
                                                           group = x$group[c(1:100, 201:300)]))
  test1.dataset <- lapply(clean.datasets, function(x) list(data = x$data[, c(101:200, 301:400)],
...
clean.data.analysis <- function(c){
  simulated_data <- clean.scenario(c)
...
#   filename <- sprintf("classification_scenario1_c%.1f.RData", c_val)
# 
#   # Process single parameter set
#   message(sprintf("Processing c=%.1f", c_val))
#   result <- clean.data.analysis(c_val)
# 
...
#   gc()
# 
#   message(sprintf("Completed and saved results for c=%.1f", c_val))
# }
...
for (i in seq_along(params)) {
  ...
  # Process single parameter set
  message(sprintf("Loading c=%.1f", c_val))
  load(filename)
  null_detect <- which(sapply(result$result$cv, is.null))
