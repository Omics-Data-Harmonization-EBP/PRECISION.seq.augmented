...
for(i in 1:12){
  # Find indices where results are NULL or contain error messages
  problem_indices <- which(sapply(dirty_results_list[[i]]$indexessummary, function(x) {
    is.null(x) || 
    (inherits(x, "try-error")) ||
    (is.list(x) && (is.null(x$ari) || is.null(x$silhouette)))
  }))
  
  if(length(problem_indices) > 0) {
    rerun_results <- mclapply(problem_indices, measure,
                             datalist = dirty_results_list[[i]]$dirtydata, 
                             mc.cores = 50)
    dirty_results_list[[i]]$indexessummary[problem_indices] <- rerun_results
  }
}
save(dirty_results_list, file = "clustering_sceanrio2.RData")
