...
classification.pam <- function(object, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
  ...
  dat.listtest1 <- lapply(object@harmon.test1.data, function(x) {
    if(any(x$dat.harmonized < 0)){
      x$dat.harmonized
    } else{
      log2(x$dat.harmonized + 1)
    }})
  dat.listtest2 <- lapply(object@harmon.test2.data, function(x) {
    if(any(x$dat.harmonized < 0)){
      x$dat.harmonized
    } else{
      log2(x$dat.harmonized + 1)
  ...
  pam <- function (datatrain,datatest1,datatest2,labeltrain, labeltest1,labeltest2, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
    ...
    pam.pred1 <- pam.predict(pam.intcv.model = pam.intcv.model, pred.obj = datatest1, pred.obj.group.id = labeltest1)
    pam.pred2 <- pam.predict(pam.intcv.model = pam.intcv.model, pred.obj = datatest2, pred.obj.group.id = labeltest2)
    
    mcexternal1 <- pam.pred1$mc
    mcexternal2 <- pam.pred2$mc

    return(c(mccv,mcexternal1,mcexternal2))
  }

  c1<-lapply(1:length(dat.listtrain),function (i) pam(dat.listtrain[[i]],dat.listtest1[[i]],dat.listtest2[[i]],
                                                      object@raw.train.data$label,object@raw.test1.data$label,object@raw.test2.data$label))
...
classification.knn <- function(object, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
  ...
  knn <- function(datatrain,datatest1,datatest2,labeltrain, labeltest1,labeltest2, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
    knn.intcv.model=knn.intcv(kfold = kfold, X=t(datatrain), y=labeltrain)
    mccv=knn.intcv.model$mc
    
    pred.obj=t(datatest1)
    pred.obj.group.id=labeltest1
    pred1 <- knn.predict(knn.intcv.model,pred.obj,pred.obj.group.id)
    mcexternal1=pred1$mc
    
    pred.obj=t(datatest2)
    pred.obj.group.id=labeltest2
    pred2 <- knn.predict(knn.intcv.model,pred.obj,pred.obj.group.id)
    mcexternal2=pred2$mc

    return(c(mccv,mcexternal1,mcexternal2))
  }


  c1<-lapply(1:length(dat.listtrain),function (i) knn(dat.listtrain[[i]],dat.listtest1[[i]],dat.listtest2[[i]],
                                                      object@raw.train.data$label,object@raw.test1.data$label,object@raw.test2.data$label))
  names(c1) <- c('TC','UQ','med','TMM','DESeq','PoissonSeq','QN','RUVg','RUVs','RUVr')
...
classification.svm <- function(object, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
  ...
  svm <- function(datatrain,datatest1,datatest2,labeltrain, labeltest1, labeltest2, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
    svm.intcv.model=svm.intcv(kfold = kfold, X=t(datatrain), y=labeltrain)
    mccv=svm.intcv.model$mc
    
    pred.obj=t(datatest1)
    pred.obj.group.id=labeltest1
    pred1 <- svm.predict(svm.intcv.model,pred.obj,pred.obj.group.id)
    mcexternal1=pred1$mc
    
    pred.obj=t(datatest2)
    pred.obj.group.id=labeltest2
    pred2 <- svm.predict(svm.intcv.model,pred.obj,pred.obj.group.id)
    mcexternal2=pred2$mc
  ...
  c1<-lapply(1:length(dat.listtrain),function (i) svm(dat.listtrain[[i]],dat.listtest1[[i]],dat.listtest2[[i]],
                                                      object@raw.train.data$label,object@raw.test1.data$label,object@raw.test2.data$label))
  names(c1) <- c('TC','UQ','med','TMM','DESeq','PoissonSeq','QN','RUVg','RUVs','RUVr')
...
classification.lasso <- function(object, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
  ...
  lasso <- function(datatrain,datatest1,datatest2,labeltrain, labeltest1, labeltest2, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
    lasso.intcv.model=lasso.intcv(kfold = kfold, X=t(datatrain), y=labeltrain)
    mccv=lasso.intcv.model$mc
    
    pred.obj=t(datatest1)
    pred.obj.group.id=labeltest1
    pred1 <- lasso.predict(lasso.intcv.model,pred.obj,pred.obj.group.id)
    mcexternal1=pred1$mc
    
    pred.obj=t(datatest2)
    pred.obj.group.id=labeltest2
    pred2 <- lasso.predict(lasso.intcv.model,pred.obj,pred.obj.group.id)
    mcexternal2=pred2$mc

    return(c(mccv,mcexternal1,mcexternal2))
  }

  c1<-lapply(1:length(dat.listtrain),function (i) lasso(dat.listtrain[[i]],dat.listtest1[[i]],dat.listtest2[[i]],
                                                        object@raw.train.data$label,object@raw.test1.data$label,object@raw.test2.data$label))
  names(c1) <- c('TC','UQ','med','TMM','DESeq','PoissonSeq','QN','RUVg','RUVs','RUVr')
...
classification.ranfor <- function(object, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
  ...
  ranfor <- function(datatrain,datatest1,datatest2,labeltrain, labeltest1, labeltest2, vt.k = NULL, n.k = 30, kfold = 5, folds = NULL){
    ranfor.intcv.model=ranfor.intcv(kfold = kfold, X=t(datatrain), y=labeltrain)
    mccv=ranfor.intcv.model$mc
    
    pred.obj=t(datatest1)
    pred.obj.group.id=labeltest1
    pred1 <- ranfor.predict(ranfor.intcv.model,pred.obj,pred.obj.group.id)
    mcexternal1=pred1$mc
    
    pred.obj=t(datatest2)
    pred.obj.group.id=labeltest2
    pred2 <- ranfor.predict(ranfor.intcv.model,pred.obj,pred.obj.group.id)
    mcexternal2=pred2$mc

    return(c(mccv,mcexternal1,mcexternal2))
  }

  c1<-lapply(1:length(dat.listtrain),function (i) ranfor(dat.listtrain[[i]],dat.listtest1[[i]],dat.listtest2[[i]],
                                                         object@raw.train.data$label,object@raw.test1.data$label,object@raw.test2.data$label))
  names(c1) <- c('TC','UQ','med','TMM','DESeq','PoissonSeq','QN','RUVg','RUVs','RUVr')
