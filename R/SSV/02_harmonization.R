...
harmon.TMM <- function(object){
  ...
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$TMM <- harmon.method.TMM(raw = object@raw.test1.data$data,
                                                     groups = object@raw.test1.data$label)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$TMM <- harmon.method.TMM(raw = object@raw.test2.data$data,
                                                      groups = object@raw.test2.data$label)
  }
  return(object)
}
...
harmon.TC <- function(object){
  object@harmon.train.data$TC <- harmon.method.TC(raw = object@raw.train.data$data,
                                                  groups = object@raw.train.data$label)
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$TC <- harmon.method.TC(raw = object@raw.test1.data$data,
                                                   groups = object@raw.test1.data$label)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$TC <- harmon.method.TC(raw = object@raw.test2.data$data,
                                                    groups = object@raw.test2.data$label)
  }
  return(object)
}
...
harmon.UQ <- function(object){
  object@harmon.train.data$UQ <- harmon.method.UQ(raw = object@raw.train.data$data,
                                                  groups = object@raw.train.data$label)
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$UQ <- harmon.method.UQ(raw = object@raw.test1.data$data,
                                                   groups = object@raw.test1.data$label)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$UQ <- harmon.method.UQ(raw = object@raw.test2.data$data,
                                                    groups = object@raw.test2.data$label)
  }
  return(object)
}
...
harmon.med <- function(object){
  object@harmon.train.data$med <- harmon.method.med(raw = object@raw.train.data$data,
                                                    groups = object@raw.train.data$label)
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$med <- harmon.method.med(raw = object@raw.test1.data$data,
                                                     groups = object@raw.test1.data$label)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$med <- harmon.method.med(raw = object@raw.test2.data$data,
                                                      groups = object@raw.test2.data$label)
  }
  return(object)
}
...
harmon.DESeq <- function(object){
  object@harmon.train.data$DESeq <- harmon.method.DESeq(raw = object@raw.train.data$data,
                                                        groups = object@raw.train.data$label)
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$DESeq <- harmon.method.DESeq(raw = object@raw.test1.data$data,
                                                         groups = object@raw.test1.data$label)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$DESeq <- harmon.method.DESeq(raw = object@raw.test2.data$data,
                                                          groups = object@raw.test2.data$label)
  }
  return(object)
}
...
harmon.PoissonSeq <- function(object){
  ...
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$PoissonSeq <- harmon.method.PoissonSeq(raw = object@raw.test1.data$data)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$PoissonSeq <- harmon.method.PoissonSeq(raw = object@raw.test2.data$data)
  }
  return(object)
}
...
harmon.QN <- function(object){
  ...
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$QN <- harmon.method.QN.frozen(rawtrain = object@raw.train.data$data,rawtest = object@raw.test1.data$data)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$QN <- harmon.method.QN.frozen(rawtrain = object@raw.train.data$data,rawtest = object@raw.test2.data$data)
  }
  return(object)
}
...
harmon.SVA <- function(object){
  object@harmon.train.data$SVA <- harmon.method.SVA(raw = object@raw.train.data$data,
                                                    groups = object@raw.train.data$label)
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$SVA <- harmon.method.SVA(raw = object@raw.test1.data$data,
                                                     groups = object@raw.test1.data$label)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$SVA <- harmon.method.SVA(raw = object@raw.test2.data$data,
                                                      groups = object@raw.test2.data$label)
  }
  return(object)
}
...
harmon.RUVg <- function(object){
  object@harmon.train.data$RUVg <- harmon.method.RUVg(raw = object@raw.train.data$data,
                                                      groups = object@raw.train.data$label)
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$RUVg <- harmon.method.RUVg(raw = object@raw.test1.data$data,
                                                       groups = object@raw.test1.data$label)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$RUVg <- harmon.method.RUVg(raw = object@raw.test2.data$data,
                                                        groups = object@raw.test2.data$label)
  }
  return(object)
}
...
harmon.RUVs <- function(object){
  object@harmon.train.data$RUVs <- harmon.method.RUVs(raw = object@raw.train.data$data,
                                                      groups = object@raw.train.data$label)
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$RUVs <- harmon.method.RUVs(raw = object@raw.test1.data$data,
                                                       groups = object@raw.test1.data$label)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$RUVs <- harmon.method.RUVs(raw = object@raw.test2.data$data,
                                                        groups = object@raw.test2.data$label)
  }
  return(object)
}
...
harmon.RUVr <- function(object){
  object@harmon.train.data$RUVr <- harmon.method.RUVr(raw = object@raw.train.data$data,
                                                      groups = object@raw.train.data$label)
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$RUVr <- harmon.method.RUVr(raw = object@raw.test1.data$data,
                                                       groups = object@raw.test1.data$label)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$RUVr <- harmon.method.RUVr(raw = object@raw.test2.data$data,
                                                        groups = object@raw.test2.data$label)
  }
  return(object)
}
...
harmon.ComBat.Seq <- function(object, batches){
  object@harmon.train.data$ComBat.Seq <- harmon.method.ComBat.Seq(raw = object@raw.train.data$data,
                                                                  batches = batches)
  if(!is.null(object@raw.test1.data)){
    object@harmon.test1.data$ComBat.Seq <- harmon.method.ComBat.Seq(raw = object@raw.test1.data$data,
                                                                   batches = batches)
  }
  if(!is.null(object@raw.test2.data)){
    object@harmon.test2.data$ComBat.Seq <- harmon.method.ComBat.Seq(raw = object@raw.test2.data$data,
                                                                    batches = batches)
  }
  return(object)
}
